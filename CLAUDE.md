...
This R package implements the Dagster Pipes protocol, enabling R scripts
to run as external processes orchestrated by Dagster. It communicates
bidirectionally with Dagster: receiving execution context (asset keys,
partition info, extras) and reporting back materializations, asset
checks, logs, and custom messages.

## Design Principles

- **Minimal dependencies**: Only {R6} and {jsonlite} are allowed as
  Imports dependencies. All other functionality must use base R only (no
  {httr}, no {curl}, etc.).
- **Single transport**: File-based message writing only
  (newline-delimited JSON appended to a temp file). This matches the
  default `PipesSubprocessClient` / `PipesTempFileMessageReader` on the
  Dagster side.
- **Graceful degradation**: When `DAGSTER_PIPES_CONTEXT` is not set
  (i.e., script is run outside Dagster),
  [`open_dagster_pipes()`](https://joekirincic.github.io/dagsterpipes/reference/open_dagster_pipes.md)
  returns a no-op context that logs to the console and silently ignores
  materializations. This allows scripts to be tested standalone.
- **R6 class system**: Use R6 for stateful objects (PipesContext,
  message writer channel).
...
for base64 decoding,
[`base::memDecompress()`](https://rdrr.io/r/base/memCompress.html) for
zlib decompression, and
...
Supported types: `"text"`, `"url"`, `"path"`, `"notebook"`, `"json"`,
`"md"`, `"float"`, `"int"`, `"bool"`, `"dagster_run"`, `"asset"`,
`"null"`, `"table"`, `"table_schema"`, `"table_column_lineage"`,
`"timestamp"`, `"__infer__"`
...
- `decode_param(env_var_value)` — Full decode pipeline:
  [`jsonlite::base64_dec()`](https://jeroen.r-universe.dev/jsonlite/reference/base64.html)
  -\> [`memDecompress()`](https://rdrr.io/r/base/memCompress.html) -\>
  [`jsonlite::fromJSON()`](https://jeroen.r-universe.dev/jsonlite/reference/fromJSON.html)
- `json_serialize(obj)` — Thin wrapper around
  [`jsonlite::toJSON()`](https://jeroen.r-universe.dev/jsonlite/reference/fromJSON.html)
  with appropriate defaults (`auto_unbox = TRUE`, `null = "null"`,
  `digits = NA`) to produce Dagster-compatible JSON
...
A lightweight R6 class that mirrors PipesContext’s interface but does
nothing. Log calls go to
...
- Use `jsonlite::fromJSON(txt, simplifyVector = FALSE)` when parsing
  context data to preserve the list-of-lists structure (avoid R
  auto-simplifying to data frames/vectors).
- Use `jsonlite::toJSON(x, auto_unbox = TRUE, null = "null")` when
  serializing messages so that single values are not wrapped in arrays
  and NULLs become JSON `null`.
- Use
  [`jsonlite::base64_dec()`](https://jeroen.r-universe.dev/jsonlite/reference/base64.html)
  for decoding bootstrap env var values.
...
- Unit test decode_param (mock base64+zlib-compressed input, verify
  correct output)
- Test PipesContext with mock context data and a temp file message
  channel
- Integration test: simulate full Dagster Pipes flow by setting env
  vars, calling
  [`open_dagster_pipes()`](https://joekirincic.github.io/dagsterpipes/reference/open_dagster_pipes.md),
  reporting a materialization, closing, then reading the messages file
  to verify correct JSON output
...
1.  `utils.R` —
    [`decode_param()`](https://joekirincic.github.io/dagsterpipes/reference/decode_param.md),
    [`json_serialize()`](https://joekirincic.github.io/dagsterpipes/reference/json_serialize.md)
    wrapper
2.  `message_writer.R` — `PipesFileMessageWriterChannel`
3.  `context.R` — `PipesContext` (depends on message writer)
4.  `open.R` —
    [`open_dagster_pipes()`](https://joekirincic.github.io/dagsterpipes/reference/open_dagster_pipes.md)
    (depends on context + utils)
5.  `metadata.R` —
